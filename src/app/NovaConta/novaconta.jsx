...
function NovaConta(){
    ...
    function cadastrarUsuario(){
        ...
        firebase.auth().createUserWithEmailAndPassword(email, senha).then(resultado => {
            ...
            setSucesso('N');
            if (error.mensagem === 'Password should be at least 6 characters'){
                setMensagem('A senha deve ter pelo menos 6 caracteres')
            } else if (error.mensagem === 'The email address is badly formatted'){
                setMensagem('O email não é válido');
            } else if (error.mensagem === 'The email address is already in use by another account'){
                setMensagem('Esse email já está um uso por outra conta');
            } else {
                setMensagem('Erro ao criar conta: ' + error.mensagem)   
            }
        });
    }
